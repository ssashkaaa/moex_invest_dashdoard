...
if not securities_df.empty:
    ...
    if tickers: #проводим инвестиционный анализ в случае, если пользователь выбрал хотя бы одну акцию
        ...
        col1.metric("Доходность инвестиционного портфеля", f"{total_return:.2%}", help="Общий рост стоимости инвестиционного портфеля за выбранный период.")
        col2.metric("Волатильность ежедневной доходности акций", f"{risk:.2%}", help="Изменчивость стоимости инвестиционного портфеля.")
        ...
        if returns.shape[1] > 1:
            ...
            st.divider()
            st.write("На этом графике вы видите **Границу эффективности** — множество портфелей с идеальным соотношением риска и доходности. Ваша задача заключается в том, чтобы найти на этом графике инвестиционный портфель, который лучше всего соответствует вашим целям.")
            st.write("\n **Случайные портфели** образуют 'облако', а каждый портфель в нём — это одна из тысяч случайных комбинаций акций. **Ваш портфель** показан чёрной точкой. \n **Оптимальный портфель** — это символ красная звезда. Это самая эффективная комбинация активов, которая даёт максимальную доходность при вашем уровне риска.")
            ...
            fig_opt.update_layout(
                ...
                legend_title_text='Инвестиционные портфели')
